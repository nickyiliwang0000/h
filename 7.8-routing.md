# Router	
If you have wireless Internet in your home, then you might already be familiar with how a router works. In your home, your router directs the data between your devices and the Internet - think about it kind of like an air traffic controller for your data.

The same concept can be applied to our React applications. Right now all of our apps have only had one 'page' to speak of (we'll call this a *view*), but what happens if we want to show multiple different views? React Router, a router package written by some people who wanted to solve this problem, can help direct our users between different views, prevent users from seeing things they shouldn't, and make sure that our application always delivers the correct view.
...
## Building a Personal Site using React Router
...
Create a new app using `create-react-app` and then install the `react-router-dom` package.
...
And now we can use `<Link >`s much the same way we use the `<a href="#" >` element:
...
With this, we can navigate between our components! Pretty amazing. You'll notice that we can use the browser's back and forward buttons and we stay within our app.
...
Adding a route that accepts URL params is easy - we just add `:paramName` to the end of our `<Route />` tag, like this:
...
```
Then, use this parameter in your About component:
...
class About extends Component {
  render() {
    return (
      <div>
        <h2>About Us</h2>
        <p>We are experts at {this.props.match.params.skill} !</p>
      </div>
    )
  }
}
...
Now try navigating to `localhost:3000/apology/anyname` and see that the content will be customized based on what name you put in! This is kind of a silly example, but I hope it gets you thinking about how params work and how information can be shared between views.
