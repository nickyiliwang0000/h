  ...
  - That align-content arranges content within the context of the flex container
  -->
...
Add a new rule to your file our [our CodePen](https://codepen.io/zkdan/pen/OaKQjx): `width: 200px;`.
...
In the previous examples, has been a lot of white space all around the elements. Some of the other properties available to us with flexbox allow us to control how the space between elements. (Note that these properties only apply when there is **more than one row or column of content** (depending on the `flex-direction`).
...
### `flex: 1 0 auto;`
Setting `flex` to `1 0 auto` is a shorthand for `flex-grow: 1;`, `flex-shrink: 0;`, and `flex-basis: auto;`. This is quite a common declaration. 
