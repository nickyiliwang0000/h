...
<!-- This p tag is how you get GFM to center something -->
<p align="center">
```html
...
Imagine that you have a header set to `height: 100vh` with a long title inside of it. On phones with short screens or small tablets in landscape mode the title might get cut off or break out of its container. Download [height-mq.html](https://hychalknotes.s3.amazonaws.com/height-mq.html) to see this issue in action. Open it in the device emulator on a small phone. See how the text runs past the image? We can solve this issue by having the header automatically size itself to its contents once the screen is below a certain height:
...
For most cases, **you will be okay using width media queries** instead of orientation. But we tell you so you know. 🙏
...
In general, we don't want to create breakpoints based on specific devices. There are hundreds of screen sizes out there now and more to come. Instead of choosing specific device sizes at which to add breakpoints, create them based on the design and content of your website. For certain edge cases where you must target specific devices, check out [this resource by Chris Coyier](https://css-tricks.com/snippets/css/media-queries-for-standard-devices/) which provides media queries for some known devices sizes. 
...
Some tips for using dev tools to check responsiveness:
  ...
  1. Dock your dev tools to the right of the screen (or in a new window completely). Click the three dots in the top right of your dev tools and select the option you want.
    
    ![screencap of the dev tools location menu on Firefox](https://hychalknotes.s3.amazonaws.com/dev-tools-location-menu.png)

  1. To get to the device emulator in your browser, click the screens icon next to the three dots.

    ![screencap of the dev tools emulator icon on Firefox](https://hychalknotes.s3.amazonaws.com/dev-tools-emulator.png)

  1. Turn on developer mode in Safari and use the same tools there.
...
Let's revisit the flexbox navigation exercise from the previous lesson! Work off of what you have so far, or download the [styled navigation zip file](https://hychalknotes.s3.amazonaws.com/navigation.zip)and make it responsive! Check out the answer key in your browser to see what it's supposed to look like [navigation-answer.zip](https://hychalknotes.s3.amazonaws.com/navigation-answer.zip).

### More exercises
Now we are going to work on exercises. Starting with easy media queries and moving on up in complexity. See [responsive-exercises.zip](https://hychalknotes.s3.amazonaws.com/responsive-exercises.zip) for the exercises and answers.