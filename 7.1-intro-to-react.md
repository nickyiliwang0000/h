...
JSX is a syntax extension for JavaScript that can be passed into React's `render` method and compiled into HTML elements. It looks a lot like HTML, but it has a few gotchas.
...
* One other gotcha is what JavaScript you can put inside of JSX. Anything that is inside of the `{}` must be an expression. This gets challenging when you only want to render a piece of your UI sometimes. 

 * You will be tempted to want to put an `if` statement in your JSX: 
  ```javascript
  render() {
    return (
      <div>
