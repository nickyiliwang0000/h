...
Your `SearchBar` component should now appear inside your `App` component!
...
Also - you'll notice that when we write out the `<SearchBar />` component that we include a `/` at the end. This is because our React Elements are also **self closing elements**. Just like in JSX, in React it is important that any time we create a component that doesn't have any children, we let React know by adding a `/`.

## Try it out yourself!
Practice making a few components and nesting them inside your `<App>` component! They can do whatever you want - this is just to practice and get used to the class component syntax.
...
In order to set up Sass when working with Create React App we need to add a few things.  You can find a write up of this in the documentation for Create React App [Here](https://github.com/facebook/create-react-app/blob/master/packages/react-scripts/template/README.md#adding-a-css-preprocessor-sass-less-etc). But let’s look at it together.
...
The `node-sass-chokidar` package is used to compile our sass, while the `npm-run-all` package is used run the development server.

Now that both of those are install we need to change the `package.json` file to incorporate these changes. Our `scripts` section should now look like this.
...
Running the `npm start` command will not start our JS and CSS watching. And that is all we need to use sass!