...
What makes a site easy to use? How do we define how our content is laid out and presented? This lesson we will discuss how to create structure and lay out our sites effectively.

We'll identify structure within existing sites and plan how to effectively lay out content using margin, padding and dimensions, and introduce two new properties called `float` and `clear`.

### Identifying Structure

Everything in HTML exists as a box. It will have dimensions such as width and height and contain content, or other boxes.

We've looked at how we can contain content in elements semantically using divs, or Semantic Elements, but the question remains 'why do we want to group objects?'. Within layout, it's much easier to move objects as a group, rather than individually.

In HTML, almost all structural elements display in the browser as items that are 100% wide. Any following elements will display underneath the preceding ones much like lines of text. These elements are what is known as 'block elements'.
...
The Internet is however not made up of items stacked on top of each other. It consists of complex layouts where articles are next to sidebars. Lists can run inline rather than stacked.

Within HTML/CSS, this is often thought to be as direct as simply changing the dimensions of objects, in order to allow items to fit. Logically, knowing that all block elements are 100% wide, simply changing the elements to be 50% should bring the next elements up next to it right?
