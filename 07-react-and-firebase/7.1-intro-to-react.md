...
Notice that we import `React` and `ReactDom` in this file. `react-dom` is a _module_ (a small set of code) needed to render our application to the DOM. (There is a version of React that can be used server-side as well.)
